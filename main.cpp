...
int main(int argc, char** argv) {
  ...
  /* Parse arguments. */
  //int SCALE = 16; defined global for bfs 
  ...
  if (tg.data_in_file) {
    ...
    char name [] = "native";
    MPI_File_set_view(tg.edgefile, 0, packed_edge_mpi_type, packed_edge_mpi_type, name, MPI_INFO_NULL);
  ...
  double make_graph_start = MPI_Wtime();
  {
	...
	//printf("%d : nrows(%d) my_row(%d) my_col(%d) my_pos(%d) block_limit(%d) FILE_CHUNCKSIZE(%d)\n",rank,nrows,my_row,my_col,my_pos,block_limit,FILE_CHUNKSIZE);
