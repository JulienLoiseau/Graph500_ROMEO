...
#define BITMAP_UNIT uint32_t
#define BITMAP_UNIT_MPI MPI_UINT32_T
...
#define FILE_CHUNKSIZE ((MPI_Offset)(1) << 23) /* Size of one file I/O block or memory block to be processed in one step, in edges ###de base 22 !###*/
...
void explore_frontier_CPU(unsigned int * sum, BITMAP_UNIT * in_queue, BITMAP_UNIT * out_queue, int32_t * visited_label, unsigned int * CSR_R, unsigned int * CSR_C, int col_num, unsigned int nwords_per_cpu, unsigned int nvertices_2d_per_gpu, unsigned int nvertices_2d_per_proc );
